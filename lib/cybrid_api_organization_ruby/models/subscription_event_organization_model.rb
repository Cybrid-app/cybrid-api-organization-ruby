...
module CybridApiOrganization
  class SubscriptionEventOrganizationModel
    ...
    # Context data for email template rendering.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiOrganization::SubscriptionEventOrganizationModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiOrganization::SubscriptionEventOrganizationModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'context_data')
        self.context_data = attributes[:'context_data']
      end
    ...
    def hash
      [guid, bank_guid, event_type, object_guid, environment, organization_guid, context_data, created_at, updated_at].hash
    end
