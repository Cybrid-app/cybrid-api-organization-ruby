...
module CybridApiOrganization
  ...
  class PostSubscriptionOrganizationModel
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiOrganization::PostSubscriptionOrganizationModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiOrganization::PostSubscriptionOrganizationModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@recipient.nil? && @recipient.to_s.length > 254
        invalid_properties.push('invalid value for "recipient", the character length must be smaller than or equal to 254.')
      end

      if !@recipient.nil? && @recipient.to_s.length < 1
        invalid_properties.push('invalid value for "recipient", the character length must be great than or equal to 1.')
      end
    ...
    def valid?
      ...
      type_validator = EnumAttributeValidator.new('String', ["webhook", "email"])
      ...
      return false if !@recipient.nil? && @recipient.to_s.length > 254
      return false if !@recipient.nil? && @recipient.to_s.length < 1
      true
    end
    ...
    def recipient=(recipient)
      if !recipient.nil? && recipient.to_s.length > 254
        fail ArgumentError, 'invalid value for "recipient", the character length must be smaller than or equal to 254.'
      end

      if !recipient.nil? && recipient.to_s.length < 1
        fail ArgumentError, 'invalid value for "recipient", the character length must be great than or equal to 1.'
      end
    ...
    def hash
      [environment, type, name, url, recipient].hash
    end
